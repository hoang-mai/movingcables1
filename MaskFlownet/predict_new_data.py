...
def find_checkpoint(checkpoint_str, args):
    # find checkpoint
    steps = 0
...
def load_model(config_str):
    ...
    with open(os.path.join(repoRoot, 'network', 'config', config_str)) as f:
        config =  network.config.Reader(yaml.load(f, Loader=yaml.SafeLoader))
    return config


def instantiate_model(gpu_device, config, args):
...
def predict_image_pair_flow(img1, img2, pipe, resize=None):
    ...
    for result in pipe.predict([img1], [img2], batch_size = 1, resize=resize):
        if len(result) == 3:
            ...
            flow, flow_var, occ_mask, warped = result
        else:
            raise RuntimeError("Invalid length of predict result:"+
                " {:d}. Expected 3 or 4.".format(len(result)))
...
def predict_video_flow(video_filename, batch_size, resize=None):
    ...
    flow_video = [res[0] for res in pipe.predict(prev_frames, new_frames, batch_size=batch_size, resize=resize)]
...
if __name__ == "__main__":
    ...
    checkpoint, steps = find_checkpoint(args.checkpoint, args)
    ...
    if args.image_1 is not None:
        ...
        flow, flow_var, occ_mask, warped = predict_image_pair_flow(image_1, image_2, pipe)
